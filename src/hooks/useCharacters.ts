...
export const useCharacters = () => {
  ...
  useEffect(() => {
    const fetchCharacters = async () => {
      ...
      try {
        ...
        dispatch({ type: 'FETCH_SUCCESS', payload: { characters: data.results, totalPages: data.info.pages } });
      } catch (error) {
        dispatch({ type: 'FETCH_ERROR', payload: 'Error al cargar los personajes' });
      }
    };
  ...
  return { ...state, setPage: (page: number) => dispatch({ type: 'SET_PAGE', payload: page }) };
};
